...
def split_columns(df):
    df['образование_код'] = df['образование_код'].astype('category')
...
def correlation_analysis(df):
    ...
    if 'зарплата' in df_numeric.columns:
        corr = df_numeric.corr()['зарплата'].sort_values(ascending=False)
        ...
        significant_corr = corr[(corr.abs() > 0.1) & (corr.index != 'зарплата')]
